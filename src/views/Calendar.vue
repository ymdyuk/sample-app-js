<template>
  <v-row class="fill-height">
    <v-col class="pt-0">
      <v-sheet height="64">
        <v-toolbar flat>
          <v-btn outlined class="mr-4" color="gray darken-2" @click="setToday">
            Today
          </v-btn>
          <v-btn fab text small color="gray darken-2" @click="prev">
...
export default defineComponent({
  ...
  setup(props, context) {
    const state = reactive({
      ...
      newEvent: null,
      // イベント開始日付を選択する'v-date-picker'コンポーネントを表示するかどうかを示す値です。
    ...
    const methods = {
      ...
      showEvent: ({ nativeEvent, event }) => {
        // 実装は https://vuetifyjs.com/components/calendars を参照
        // setTimeout を使用して遅延させないと、メニューを正しく表示できない。
        const open = () => {
          state.selectedEvent = event;
          state.menuActivatorEvent = nativeEvent.target;
          setTimeout(() => {
       ...
       * カレンダーが描画されたタイミング、または表示範囲が変更されたタイミングで呼び出します。
       */
